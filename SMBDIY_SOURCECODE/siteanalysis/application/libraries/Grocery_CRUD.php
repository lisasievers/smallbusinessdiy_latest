		...
		switch ($real_type) {
			...
			case 'date':
				if(!empty($value) && $value != '0000-00-00' && $value != '1970-01-01')
				{
					...
					//$value = date($this->php_date_format, mktime (0, 0, 0, (int)$day , (int)$month , (int)$year));
			...
			case 'datetime':
				if(!empty($value) && $value != '0000-00-00 00:00:00' && $value != '1970-01-01 00:00:00')
				{
					...
					//$value = date($this->php_date_format." - H:i", mktime ((int)$hours , (int)$minutes , 0, (int)$day , (int)$month ,(int)$year));
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function _print_webpage($data)
	{
		...
		#print-table{ color: #000; background: #fff; font-family: Verdana,Tahoma,Helvetica,sans-serif; font-size: 12px;}
	...
	// for pdf generator -- starts here --
	protected function pdf_webpage($state_info = null)
	{
		$data = $this->get_common_data();
		...
		$data->primary_key 			= $this->get_primary_key();
		echo "<pre>";
		print_r($data->primary_key);
		echo "<pre>";

		@ob_end_clean();
		$this->_pdf_webpage($data);
	}

	protected function _pdf_webpage($data)
	{
		$string_to_print = "<meta charset=\"utf-8\" /><style type=\"text/css\" >
		#print-table{ color: #000; background: #fff; font-family: Verdana,Tahoma,Helvetica,sans-serif; font-size: 12px;}
		#print-table table tr td, #print-table table tr th{ border: 1px solid black; border-bottom: none; border-right: none; padding: 4px 8px 4px 4px}
	...
	protected function showAddForm()
	{
		...
		// added by al-amin to load user defined add form
		$is_consumer_template=0;
		$CI =& get_instance();
		$CI->load->library('session');
		if($CI->session->userdata('is_consumer_template')!='')
		$is_consumer_template=$CI->session->userdata('is_consumer_template');
		$CI->session->set_userdata('is_consumer_template',0);
		if($is_consumer_template==1)
		$this->_theme_view('add_consumer.php',$data);
		else
		// end of addition
	...
	protected function showEditForm($state_info)
	{
		...
		$CI->session->set_userdata('is_consumer_template',0);
		if($is_consumer_template==1)
		$this->_theme_view('edit_consumer.php',$data);
		else
		// end of addition
	...
	protected function get_datetime_input($field_info,$value)
	{
		...
		if(!empty($value) && $value != '0000-00-00 00:00:00' && $value != '1970-01-01 00:00:00'){
			...
			// $date = date($this->php_date_format, mktime(0,0,0,$day,$month,$year));
	...
	protected function get_date_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$date' maxlength='10' class='datepicker-input' data-date-format='dd/mm/yyyy' />
	...
	public function getStateInfo()
	{
		...
		switch ($state_code) {
			...
			case 17: //print
				...
				if($state_code === 16 || $state_code === 17 || $state_code === 19)
				{
					$state_info->page = 1;
				...
				if(!empty($_POST['search_text']))
				{
					if(empty($_POST['search_field']))
					{
						...
						$state_info->search = (object)array( 'field' => null , 'text' => trim($_POST['search_text']) );

					}
					else
					{
						$state_info->search	= (object)array( 'field' => strip_tags($_POST['search_field']) , 'text' => trim($_POST['search_text']) );
					}
				}
			break;
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $default_language_path				= 'plugins/grocery_crud/languages';
	protected $default_config_path					= 'plugins/grocery_crud/config';
	protected $default_assets_path					= 'plugins/grocery_crud';
	...
	/**
	 * Sets a custom link (added by al-amin)
	 *
	 * @return	void
	 */
	public function set_custom_link($array2D)
	{
		$CI =& get_instance();
		$CI->load->library('session');
		$CI->session->set_userdata('set_custom_link',$array2D);
	}
...
//@todo ui_day, ui_month, ui_year has to be lang strings
	...
	public function set_field_upload($field_name, $upload_dir = '')
	{
		$upload_dir = !empty($upload_dir) && substr($upload_dir,-1,1) == '/'
						...
						: $upload_dir;
		$upload_dir = !empty($upload_dir) ? $upload_dir : 'plugins/uploads/files';
