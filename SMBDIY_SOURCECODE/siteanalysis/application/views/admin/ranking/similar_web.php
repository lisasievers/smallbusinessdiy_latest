...
<section class="content-header">
	<h1 class = 'text-info'>  <?php echo $this->lang->line("similarweb data");?></h1>
</section>
	...
	<div class="row" >
		<div class="col-xs-12">
			<div class="grid_container" style="width:100%; min-height:760px;">
				...
				url="<?php echo base_url()."rank/similar_web_data"; ?>" 
				...
				>
					...
					<thead> 	 	 	
						<tr>
							...
							<th field="global_rank" sortable="true">Traffic Rank</th>
							<th field="country" sortable="true">Country</th>
							<th field="country_rank" sortable="true">Country Rank</th>
							<th field="bounce_rate" sortable="true">Bounce Rate</th>
							<th field="total_visit" sortable="true">Total Visit</th>
							<th field="time_on_site" sortable="true">Time On Site</th>
							<th field="page_views" sortable="true">Page Views</th>							
							<th field="status" sortable="true">Status</th>
							<th field="searched_at" sortable="true">Search at</th>
							<th field="action" formatter="view_similarweb_data"><?php echo $this->lang->line("actions");?></th>
						</tr>
					</thead>
				</table>                        
			</div>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 id="new_search_details_title" class="modal-title"><i class="fa fa-table"></i> <?php echo $this->lang->line("similarweb data");?></h4>
			</div><br/>
...
<script>
	...
	$(".download").click(function(){
		...
		var url = "<?php echo site_url('rank/similar_web_download');?>";
	...
	$(".delete").click(function(){
		...
		if(result)
		{
			...
			var base_url="<?php echo base_url(); ?>";		
			var url = "<?php echo site_url('rank/similar_web_delete');?>";
	        var rows = $j("#tt").datagrid("getSelections");
	        var info=JSON.stringify(rows); 
	...
	function get_bulk_progress()
	{
		var base_url="<?php echo base_url(); ?>";			
		$.ajax({
			url:base_url+'rank/bulk_similar_web_progress_count',
			type:'POST',
			dataType:'json',
			success:function(response){
				...
				if(width==100) 
				{
					...
					$("#download_div").html('<center><a style="margin: 0px auto;" href="<?php echo base_url()."download/rank/similar_web_".$this->user_id."_".$this->session->userdata("download_id").".csv"; ?>" target="_blank" class="btn btn-lg btn-warning"><i class="fa fa-cloud-download"></i> <b><?php echo $this->lang->line("download"); ?></b></a></center>');
	...
	$j("document").ready(function(){
		...
		$("#new_search_button").on('click',function(){
			...
			$.ajax({
				url:base_url+'rank/similar_web_action',
				type:'POST',
				data:{urls:urls},
				success:function(response){		
	...
	function view_similarweb_data(value,row,index)
	{   	   
	    var base_url="<?php echo base_url();?>";
	    var link=base_url+"rank/similar_web_details/"+row.id;
	    return "<a href='"+link+"' class='btn btn-sm btn-primary'><i class='fa fa-table'></i><?php echo $this->lang->line('more info'); ?></a>";         
	}   
					...
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2">
							...
							<?php 
								...
								echo "<a href='".base_url()."download/rank/similar_web_".$this->user_id."_".$download_id.".csv"."'". "title='Download' class='btn btn-warning btn-lg' style='width:200px;'><i class='fa fa-cloud-download' style='color:white'></i> ".$this->lang->line('download')."</a>";							
							?>
							</div>		
