...
<section class="content-header">
	<h1 class = 'text-info'> <?php echo $this->lang->line("backlink generator");?></h1>
</section>
	...
	<div class="row" >
		<div class="col-xs-12">
			<div class="grid_container" style="width:100%; min-height:760px;">
				...
				url="<?php echo base_url()."backlink/backlink_gererator_data"; ?>" 
				...
				>
					...
					<thead>
						<tr>
							...
							<th field="url" sortable="true">Backlink URL</th>
							<th field="domain_name" sortable="true">Domain Name</th>
							<th field="response_code" sortable="true">Response Code</th>
							<th field="status" sortable="true" >Status</th>
							<th field="generated_at" sortable="true">Generated at</th>
						</tr>
					</thead>
				</table>                        
			</div>
			...
			<div class="row">
				<div class="col-xs-12">
					<button type="button" style="width:200px;" class="btn btn-primary" id ="new_search_modal_open"><i class="fa fa-undo"></i> <?php echo $this->lang->line("backlink generator");?></button>
				</div>
			</div>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 id="new_search_details_title" class="modal-title"><i class="fa fa-undo"></i> <?php echo $this->lang->line("backlink generator");?></h4>
			</div><br/>
				...
				<form enctype="multipart/form-data" method="post" class="form-inline" id="new_search_form" style="margin-bottom:10px">
					<div class="row">						
						<div class="form-group col-xs-12 col-sm-12 col-md-12 col-lg-12">
							...
							<br/>
							<!-- <span style="margin-left:5%">OR</span> -->
							<br/> 
						</div>
					</div>
					...
					<div class="row">						
						...
						<div class="form-group col-xs-12 col-sm-12 col-md-7 col-lg-7 clearfix">						
							...
							<button type="button"  id="new_search_button" class="btn btn-info pull-right"><i class="fa fa-undo"></i> <?php echo $this->lang->line("start generation");?></button>   
						</div>
					</div>
...
<script>
	...
	$(".download").click(function(){
		...
		var url = "<?php echo site_url('backlink/backlink_gererator_download');?>";
	...
	$(".delete").click(function(){
		...
		if(result)
		{
			...
			var base_url="<?php echo base_url(); ?>";		
			var url = "<?php echo site_url('backlink/backlink_gererator_delete');?>";
	        var rows = $j("#tt").datagrid("getSelections");
	        var info=JSON.stringify(rows); 
	...
	function get_bulk_progress()
	{
		var base_url="<?php echo base_url(); ?>";			
		$.ajax({
			url:base_url+'backlink/bulk_backlink_gererator_progress_count',
			type:'POST',
			dataType:'json',
			success:function(response){
				...
				$("#progress_msg_text").html(search_complete +" / "+ search_total +" <?php echo $this->lang->line('backlink generated'); ?>");
				...
				if(width==100) 
				{
					...
					$("#download_div").html('<center><a style="margin: 0px auto;" href="<?php echo base_url()."download/backlink/backlink_gererator_".$this->user_id."_".$this->session->userdata("download_id").".csv"; ?>" target="_blank" class="btn btn-lg btn-warning"><i class="fa fa-cloud-download"></i> <b><?php echo $this->lang->line("download"); ?></b></a></center>');
	...
	$j("document").ready(function(){
		...
		$("#new_search_button").on('click',function(){
			...
			$.ajax({
				url:base_url+'backlink/backlink_gererator_action',
				type:'POST',
				data:{urls:urls},
				success:function(response){		
					...
					$("#progress_msg_text").html("<?php echo $this->lang->line('backlink generated'); ?>");
					$("#success_msg").html('<center><h3 style="color:olive;"><?php echo $this->lang->line("backlink generated"); ?></h3></center>');
					$("#download_div").html('<center><a style="margin: 0px auto;" href="<?php echo base_url()."download/backlink/backlink_gererator_".$this->user_id."_".$this->session->userdata("download_id").".csv"; ?>" target="_blank" class="btn btn-lg btn-warning"><i class="fa fa-cloud-download"></i> <b><?php echo $this->lang->line("download"); ?></b></a></center>');
					$j("#tt").datagrid('reload');					
				}
					...
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2">
							...
							<?php 
								...
								echo "<a href='".base_url()."download/backlink/backlink_gererator_".$this->user_id."_".$download_id.".csv"."'". "title='Download' class='btn btn-warning btn-lg' style='width:200px;'><i class='fa fa-cloud-download' style='color:white'></i> ".$this->lang->line('download')."</a>";							
							?>
							</div>		
