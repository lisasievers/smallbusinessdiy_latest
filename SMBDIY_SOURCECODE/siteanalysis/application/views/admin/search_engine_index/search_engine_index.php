...
<section class="content-header">
	<h1 class = 'text-info'> <?php echo $this->lang->line("search engine index"); ?></h1>
</section>
	...
	<div class="row" >
		<div class="col-xs-12">
			<div class="grid_container" style="width:100%; min-height:760px;">
				...
				url="<?php echo base_url()."search_engine_index/search_engine_index_data"; ?>" 
				...
				>
					...
					<thead>
						<tr>
							...
							<th field="google_index" sortable="true">Google Index</th>
							<th field="bing_index" sortable="true">Bing Index</th>
							<th field="yahoo_index" sortable="true">Yahoo Index</th>
							<th field="checked_at" sortable="true"><?php echo $this->lang->line("search at");?></th>
						</tr>
					</thead>
				</table>                        
			</div>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 id="new_search_details_title" class="modal-title"><i class="fa fa-binoculars"></i> <?php echo $this->lang->line("search engine index");?></h4>
			</div><br/>
			...
			<div id="new_search_view_body" class="modal-body">
				<form enctype="multipart/form-data" method="post" class="form-inline" id="new_search_form" style="margin-bottom:10px">
					...
					<div class="row">
						<div class="col-xs-4">
							<label><input type="checkbox" id="google_index" value="google" checked="true"> Google</label>
						</div>
						<div class="col-xs-4">
							<label><input type="checkbox" id="bing_index" value="bing" checked="true"> Bing</label>
						</div>
						<div class="col-xs-4">
							<label><input type="checkbox" id="yahoo_index" value="yahoo" checked="true"> Yahoo</label>
						</div>	
					</div>
					<br/>
					<div class="row">						
...
<script>
	...
	$(".delete").click(function(){
		...
		if(result)
		{
			...
			var base_url="<?php echo base_url(); ?>";		
			var url = "<?php echo site_url('search_engine_index/search_engine_index_delete');?>";
	        var rows = $j("#tt").datagrid("getSelections");
	        var info=JSON.stringify(rows); 
	...
	function get_bulk_progress()
	{
		var base_url="<?php echo base_url(); ?>";			
		$.ajax({
			url:base_url+'search_engine_index/bulk_search_engine_index_progress_count',
			type:'POST',
			dataType:'json',
			success:function(response){
				...
				if(width==100) 
				{
					...
					$("#download_div").html('<center><a style="margin: 0px auto;" href="<?php echo base_url()."download/search_engine_index/search_engine_index_".$this->user_id."_".$this->session->userdata("download_id").".csv"; ?>" target="_blank" class="btn btn-lg btn-warning"><i class="fa fa-cloud-download"></i> <b><?php echo $this->lang->line("download"); ?></b></a></center>');
	...
	$j("document").ready(function(){
		...
		$("#new_search_button").on('click',function(){
			...
			var is_google=0;
			if ($('#google_index').is(":checked"))
			var is_google=1;

			var is_bing=0;
			if ($('#bing_index').is(":checked"))
			var is_bing=1;

			var is_yahoo=0;
			if ($('#yahoo_index').is(":checked"))
			var is_yahoo=1;
			...
			$.ajax({
				...
				type:'POST',
				data:{urls:urls,is_google:is_google,is_bing:is_bing,is_yahoo:is_yahoo},
				success:function(response){		
					...
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2">
							...
							<?php 
								...
								echo "<a href='".base_url()."download/search_engine_index/search_engine_index_".$this->user_id."_".$download_id.".csv"."'". "title='Download' class='btn btn-warning btn-lg' style='width:200px;'><i class='fa fa-cloud-download' style='color:white'></i> ".$this->lang->line('download')."</a>";							
							?>
							</div>		
