...
<section class="content-header">
	<h1 class = 'text-info'> <?php echo $this->lang->line("ipv6 compability check");?></h1>
</section>
	...
	<div class="row" >
		<div class="col-xs-12">
			<div class="grid_container" style="width:100%; min-height:760px;">
				...
				url="<?php echo base_url()."ip/ipv6_check_data"; ?>" 
				...
				>
					...
					<thead>
						<tr>
							...
							<th field="ip" sortable="true">IP</th>
							<th field="ipv6" formatter="ipv6_formatter" sortable="true">IPv6</th>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 id="new_search_details_title" class="modal-title"><i class="fa fa-map-marker"></i> <?php echo $this->lang->line("ipv6 compability check");?></h4>
			</div><br/>
...
<script>
	...
	function ipv6_formatter(value,row,index)
	{   
	    if(row.is_ipv6_support=="1") return "<label class=''>"+value+"</label>";            
	    else return "<label class='label label-warning'><i class='fa fa-remove'></i> Not Compatible</label>";            
	}  
	...
	$(".download").click(function(){
		...
		var url = "<?php echo site_url('ip/ipv6_check_download');?>";
	...
	$(".delete").click(function(){
		...
		if(result)
		{
			...
			var base_url="<?php echo base_url(); ?>";		
			var url = "<?php echo site_url('ip/ipv6_check_delete');?>";
	        var rows = $j("#tt").datagrid("getSelections");
	        var info=JSON.stringify(rows); 
	...
	function get_bulk_progress()
	{
		var base_url="<?php echo base_url(); ?>";			
		$.ajax({
			url:base_url+'ip/bulk_ipv6_check_progress_count',
			type:'POST',
			dataType:'json',
			success:function(response){
				...
				if(width==100) 
				{
					...
					$("#download_div").html('<center><a style="margin: 0px auto;" href="<?php echo base_url()."download/ip/ipv6_".$this->user_id."_".$this->session->userdata("download_id").".csv"; ?>" target="_blank" class="btn btn-lg btn-warning"><i class="fa fa-cloud-download"></i> <b><?php echo $this->lang->line("download"); ?></b></a></center>');
	...
	$j("document").ready(function(){
		...
		$("#new_search_button").on('click',function(){
			...
			$.ajax({
				url:base_url+'ip/ipv6_check_action',
				type:'POST',
				data:{urls:urls},
				success:function(response){		
					...
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2">
							...
							<?php 
								...
								echo "<a href='".base_url()."download/ip/ipv6_".$this->user_id."_".$download_id.".csv"."'". "title='Download' class='btn btn-warning btn-lg' style='width:200px;'><i class='fa fa-cloud-download' style='color:white'></i> ".$this->lang->line('download')."</a>";							
							?>
							</div>		
