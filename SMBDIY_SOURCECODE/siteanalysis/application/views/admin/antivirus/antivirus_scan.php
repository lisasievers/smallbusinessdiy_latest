...
<section class="content-header">
	<h1 class = 'text-info'> <?php echo $this->lang->line("malware scan");?></h1>
</section>
<section class="content">  
	<div class="row" >
		<div class="col-xs-12">
				...
				>
					...
					<thead>
						<tr>
							...
							<th field="google_status" sortable="true">Google Status</th>
							<th field="macafee_status" sortable="true">McAfee Status</th>
							<th field="avg_status" sortable="true">AVG Status</th>
							<th field="norton_status" sortable="true">Norton Status</th>
							<th field="scanned_at" sortable="true">Scanned at</th>
						</tr>
					</thead>
				</table>                        
			</div>
			...
			<div class="row">
				<div class="col-xs-12">
					<button type="button" style="width:140px;" class="btn btn-primary" id ="new_search_modal_open"><i class="fa fa-search"></i> <?php echo $this->lang->line("new scan");?></button>
				</div>
			</div>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 id="new_search_details_title" class="modal-title"><i class="fa fa-shield"></i> <?php echo $this->lang->line("malware scan");?></h4>
			</div><br/>
				...
				<form enctype="multipart/form-data" method="post" class="form-inline" id="new_search_form" style="margin-bottom:10px">
					...
					<div class="row">
						<div class="col-xs-6 col-sm-6 col-md-3 col-lg-3">
							<label><input type="checkbox" id="antivirus_google" value="google" checked="true"> Google</label>
						</div>
						<div class="col-xs-6 col-sm-6 col-md-3 col-lg-3">
							<label><input type="checkbox" id="antivirus_mcafee" value="mcafee" checked="true"> McAfee</label>
						</div>
						<div class="col-xs-6 col-sm-6 col-md-3 col-lg-3">
							<label><input type="checkbox" id="antivirus_avg" value="avg" checked="true"> AVG</label>
						</div>
						<div class="col-xs-6 col-sm-6 col-md-3 col-lg-3">
							<label><input type="checkbox" id="antivirus_norton" value="norton" checked="true"> Norton</label>
						</div>	
					</div>
					<br/>
					<div class="row">						
					...
					<div class="row">						
						...
						<div class="form-group col-xs-12 col-sm-12 col-md-7 col-lg-7 clearfix">						
							<button class='btn btn-success'  id ="pull_data"><i class="fa fa-upload"></i> </i> <?php echo $this->lang->line("upload file"); ?></button>     
							<button type="button"  id="new_search_button" class="btn btn-info pull-right"><i class="fa fa-search"></i> <?php echo $this->lang->line("start scanning"); ?></button>   
						</div>
					</div>
...
<script>
	...
	$(".download").click(function(){
		...
		var url = "<?php echo site_url('antivirus/scan_download');?>";
	...
	$(".delete").click(function(){
		...
		if(result)
		{
			...
			var base_url="<?php echo base_url(); ?>";		
			var url = "<?php echo site_url('antivirus/scan_delete');?>";
	        var rows = $j("#tt").datagrid("getSelections");
	        var info=JSON.stringify(rows); 
	...
	function get_bulk_progress()
	{
		var base_url="<?php echo base_url(); ?>";			
		$.ajax({
			url:base_url+'antivirus/bulk_scan_progress_count',
			type:'POST',
			dataType:'json',
			success:function(response){
				...
				if(width==100) 
				{
					...
					$("#download_div").html('<center><a style="margin: 0px auto;" href="<?php echo base_url()."download/antivirus/antivirus_".$this->user_id."_".$this->session->userdata("download_id").".csv"; ?>" target="_blank" class="btn btn-lg btn-warning"><i class="fa fa-cloud-download"></i> <b><?php echo $this->lang->line("download"); ?></b></a></center>');
	...
	$j("document").ready(function(){
		...
		$("#new_search_button").on('click',function(){
			...
			var is_google=0;
			if ($('#antivirus_google').is(":checked"))
			var is_google=1;

			var is_mcafee=0;
			if ($('#antivirus_mcafee').is(":checked"))
			var is_mcafee=1;

			var is_avg=0;
			if ($('#antivirus_avg').is(":checked"))
			var is_avg=1;

			var is_norton=0;
			if ($('#antivirus_norton').is(":checked"))
			var is_norton=1;
			...
			if(is_google==0 && is_mcafee==0 && is_avg==0 && is_norton==0)
			{
				alert("<?php echo $this->lang->line('please select antivirus'); ?>");
				return false;
			}
			...
			$.ajax({
				url:base_url+'antivirus/scan_action',
				type:'POST',
				data:{urls:urls,is_google:is_google,is_mcafee:is_mcafee,is_avg:is_avg,is_norton:is_norton},
				success:function(response){		
					...
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2">
							...
							<?php 
								...
								echo "<a href='".base_url()."download/antivirus/antivirus_".$this->user_id."_".$download_id.".csv"."'". "title='Download' class='btn btn-warning btn-lg' style='width:200px;'><i class='fa fa-cloud-download' style='color:white'></i> ".$this->lang->line('download')."</a>";							
							?>
							</div>		
