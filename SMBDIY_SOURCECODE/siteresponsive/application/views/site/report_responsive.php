<!-- Report view only for  Audit -->
<div class="result-container">
<style>
.audrepo{display: none;}
</style>
	<?php 
	if($load_css_js==1) 
	{
	...
	<div class="space"></div>
	<div class="space"></div>
<h3 class="resrepo_result text-center"></h3>
	<?php 
	if($load_css_js==1) 
	{
	...
	<div class="container">
		...
		<?php if($load_css_js!=1) { ?>
			<!--<div class="row">
				...
				--><!--<div class="col-xs-12 col-sm-12 col-lg-2 col-md-2 share-sibing text-center"><a id="download_list" class="btn btn-lg btn-success"> <i class="fa fa-cloud-download"></i> <?php //echo $this->lang->line("download pdf");?></a></div>
				<!--<a href="#exportModal" id="exportPage" data-toggle="modal" class="btn btn-info btn-embossed pull-right actionButtons" style="margin-right: 1px;"><span class="fui-export"></span> Export</a>
				<div class="col-xs-12">
		...
		//echo "<div style='font-size:20px !important;' class='text-center alert alert-".$warning_class."'><b>".$this->config->item('product_short_name')."</b> found <b>".$warning_count."</b> major issues</div>"; 
		echo "<div style='font-size:20px !important;' class='text-center alert alert-".$warning_class."'><b>".$this->config->item('product_short_name')."</b> estimated score is <b>".$score."</b></div>"; 
				...
				<div class="box box-<?php echo $class;?>">
					...
					<div class="box-body">	
						<div class="row">
							...
							<div class="col-xs-12 col-md-3" style="margin-bottom:30px !important;padding-left:12px;min-height:530px;background: url('<?php echo base_url("assets/images/mobile.png");?>') no-repeat !important;">
								<?php 
								$mobile_ready_data=json_decode($site_info[0]["mobile_ready_data"],true);
...
<div class="modal fade" id="exportModal" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <form class="form-horizontal">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
					<button type="button" class="close" style="display:none" data-dismiss="modal" aria-hidden="true"> <i class="fa fa-close fa-2x"></i> </button>
					<h3 class="modal-title"> <i class="fa fa-paper-plane"></i> <?php echo $this->config->item('mail_capture_label'); ?></h3>
				</div>
                    <div class="box box-green no_radius" id="subscribe_usr">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" type="button" class="btn btn-primary" id="exportSubmit">Send</button>
                    </div>
                	</div> <!-- response -->
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
...
<div class="modal fade" id="msgModal" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <form class="form-horizontal" style="min-height:120px;">
            <div class="modal-dialog" style="border:1px solid #f0ad4e;padding:1px;">
                <div class="modal-content">
                   <h2> <i class="fa fa-info-circle fa-2x"></i><br> Would you rather remain ignorant of your site's performance?</h2>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
        </form>
    </div><!-- /.modal -->
...
$(document).ready(function(){
	...
	$('.result-container').css('opacity','0.2');
	...
	$(document).on('click','#download_list',function(){
		...
		{			
			...
			$.ajax({
			url:base_url+'health_check/direct_download_responsive',
			type:'POST',
	...
	//	{			
			...
			var base_url="<?php echo site_url();?>";
			$.ajax({
			url:base_url+'/health_check/have_site_users',
			type:'POST',
			data:{resname:resname,resemail:resemail,hidden_id:hidden_id,report:'responsive'},
			success:function(res){
				$('.result-container').css('opacity','1');
				if(res==1){
					$('.close').click();
					$('.resrepo_result').html('Thanks for register with us!. <br/> For more reports please Sign in, and Get Unlimited Reports.<br/> Your Signup information sent to your registered Email. Please Check!');
					}
					else
					{
					$('.close').click();
					$('.resrepo_result').html('Welcome back!. <br/>As you registered with us already, please check your email for signup information.<br/> For more reports please Sign in, and Get Unlimited Reports.<br/>');
				}
			  }
		   });	
		//}

		//else $("#subscribe_usr").css("display","block");
	});
...
setTimeout(function(){
   var base_url="<?php echo site_url();?>";
  window.location.href=base_url+"/home/havesite";
},delay);
}
