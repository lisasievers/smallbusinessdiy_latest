...
public function get_data_sites($userid) //selects data from a table as well as counts number of affected rows
	{
		$this->db->select('site_check_report.id');
		$this->db->select('site_check_report.domain_name');
		$this->db->select('site_check_report.searched_at');
		$this->db->from('site_check_report');
		$this->db->where('site_check_report.user_id', $userid);
		$query = $this->db->get();
	...
	public function get_site_name($siteid) //selects data from a table as well as counts number of affected rows
	{
		$this->db->select('site_check_report.id');
		$this->db->select('site_check_report.domain_name');
		$this->db->select('site_check_report.searched_at');
		$this->db->from('site_check_report');
		$this->db->where('site_check_report.id', $siteid);
		$query = $this->db->get();
		if(count($query->result()) == 0)
		{
			return array();
...
public function get_user_sites($userid) //selects data from a table as well as counts number of affected rows
	{
		$this->db->select('websites.id');
		$this->db->select('websites.site_name');
		$this->db->from('websites');
		$this->db->where('websites.user_id', $userid);
		$query = $this->db->get();
		if(count($query->result()) == 0)
		{
			return array();
