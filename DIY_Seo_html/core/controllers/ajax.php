...
if ($_SERVER['REQUEST_METHOD'] =='POST')
{
    ...
    $user_email = raino_trim($_POST['email']);
    $subject = raino_trim($_POST['subject']);   
    ...
    if($name != "" || $user_email != "" || $message != ""){
        if(isValidEmail($user_email)){
            ...
            $subject = $lang['326'];
            $emailTo = $email;
            $body = $lang['235'].": $name <br /><br /> ".$lang['236'].": $user_email <br /><br /> ".$lang['238'].": ".$message;
            $subject_name = $lang['325']." $name";
            ...
            if ($mail_type == '1') {
                default_mail($user_email, $subject_name, $emailTo, $subject, $body);
            } else {
                smtp_mail($smtp_host, $smtp_port, $smtp_auth, $smtp_user, $smtp_pass, $smtp_sec,
                    $user_email, $subject_name, $emailTo, $subject, $body);
            }
    ...
    if(isset($_POST['blogPing'])){
        if(isset($_POST['pingUrl'])){
            ...
            $errorMsg = $lang['221'];
            $outData = xmlRpcPing($pingUrl,$myBlogName,$myBlogUrl,$myBlogUpdateUrl,$myBlogRSSFeedUrl,$errorMsg);
...
//Get Pagerank
if(isset($_GET['getPR'])) {
    $site = raino_trim($_GET['getPR']);
    $site = clean_url($site); $site = "http://www.$site";
    $site = parse_url(Trim($site));
    $host = $site['host'];
    $rank = google_page_rank($host);
    $pr = ($rank == '' ? '0' : $rank);
    $file = HEL_DIR."pagerank/50x/pr". Trim($pr[$loop]).".png";
...
//Script Debug - Only Authenticated Users
...
if(isset($_GET['appinfo'])){
    ...
    <table>
        <tbody>
            ...
            <tr><td>PHP Version: </td><td>'. phpversion() .' <a href="?route=ajax&phpinfo&itemCode='.$itemCode.'" target="_blank">(View PHP Info)</a></td></tr>
...
if(isset($_POST['getMobileFriendly'])){
    ...
    if($mobileScoreData != ""){
    if($isMobileFriendly){
        $isMobileFriendlyMsg = 'Awesome! This page is mobile-friendly!';
        $isMobileFriendlyColor = "2cc36b";
    }
    else{
        $isMobileFriendlyMsg = 'Oh No! This page is not mobile-friendly.';
    ...
    echo '              <div class="mobileRes">
                        <br />         
                         <table class="table table-bordered">
                            ...
                            <tr>
                                <td><strong>Mobile Friendly Score</strong></td>
                                <td>      
                            ...
                            <tr>
                                <td><strong>Mobile Screenshot of your WebPage</strong></td>
